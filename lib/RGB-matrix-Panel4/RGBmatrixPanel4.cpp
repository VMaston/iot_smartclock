/*
RGBmatrixPanel4 Arduino library for P10 1/4 scan panels with 'snake pattern' LED layout, by Ruud van Reenen.
Tested with Arduino ATMega2560 and waterproof high brightness outdoor panels ordered from AliExpress: https://goo.gl/ub1KkE

Note that initializing the panel takes an extra parameter for the number of panels.
For example: RGBmatrixPanel4 matrix(A, B, C, CLK, LAT, OE, true, 2); 		
Where 2 is the number of panels. true = for double buffering: this reduces flickering but uses twice the amount of memory.

Based on the RGBmatrixPanel4 Arduino library for Adafruit 16x32 and 32x32 RGB LED
matrix panels by Limor Fried/Ladyada & Phil Burgess/PaintYourDragon for
Adafruit Industries and the multipanel fork by protomaster:

RGBmatrixPanel4 Arduino library for Adafruit 16x32 and 32x32 RGB LED
...
// multiple RGBmatrixPanel4 instances.  The solution at present is to
...
static RGBmatrixPanel4 *activePanel = NULL;

// Code common to both the 16x32 and 32x32 constructors:
void RGBmatrixPanel4::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
...
//  static const uint8_t defaultrgbpins[] = { 2,3,4,5,6,7 };
  static const uint8_t defaultrgbpins[] = { 5,17,18,19,16,25 };
...
#endif
#endif
	nMultiplexRows = 2;	// Used to double the rowlength for 1/4 scan panels, nRows will be 4 then for 16 led high display
	rows = rows/nMultiplexRows;
	nRows = rows; // Number of multiplexed rows; actual height is 4X for 1/4 scan
	nPanels = pwidth;
	
	// Allocate and initialize matrix buffer:
	int buffsize  = 32 * nMultiplexRows * nRows * 3 * nPanels , // x3 = 3 bytes holds 4 planes "packed"
...
RGBmatrixPanel4::RGBmatrixPanel4(
...
void RGBmatrixPanel4::begin(void)
{
  ...
  pinMode(_latch, OUTPUT); *latport   &= ~latmask;   // Low
  ...
  PortType rgbmask[6];
  sclkpin = rgbclkmask = digitalPinToBitMask(_clk);
  for(uint8_t i=0; i<6; i++) {
  ...
  if (_sclk < 32) {
    ...
    PortType rgbmask[6];
    sclkpin = rgbclkmask = digitalPinToBitMask(_sclk);
    for(uint8_t i=0; i<6; i++) {
      pinMode(rgbpins[i], OUTPUT);
      rgbmask[i]  = digitalPinToBitMask(rgbpins[i]); // Pin bit mask
      rgbclkmask |= rgbmask[i];                      // Add to RGB+CLK bit mask
...
// Original RGBmatrixPanel4 library used 3/3/3 color.  Later version used
...
uint16_t RGBmatrixPanel4::Color333(uint8_t r, uint8_t g, uint8_t b)
{
	// RRRrrGGGgggBBBbb
...
uint16_t RGBmatrixPanel4::Color444(uint8_t r, uint8_t g, uint8_t b)
{
	// RRRRrGGGGggBBBBb
...
uint16_t RGBmatrixPanel4::Color888(uint8_t r, uint8_t g, uint8_t b)
{
	return ((r & 0xF8) << 11) | ((g & 0xFC) << 5) | (b >> 3);
}

// 8/8/8 -> gamma -> 5/6/5
uint16_t RGBmatrixPanel4::Color888(
    uint8_t r, uint8_t g, uint8_t b, boolean gflag)
{
	if(gflag)   // Gamma-corrected color?
	{
		r = gamma_table[r]; // Gamma correction table maps
		g = gamma_table[g]; // 8-bit input to 4-bit output
		b = gamma_table[b];
...
uint16_t RGBmatrixPanel4::ColorHSV(
    long hue, uint8_t sat, uint8_t val, boolean gflag)
{
	...
	if(gflag)   // Gamma-corrected color? 
	{
		r = gamma_table[(r * v1) >> 8]; // Gamma correction table maps
		g = gamma_table[(g * v1) >> 8]; // 8-bit input to 4-bit output
		b = gamma_table[(b * v1) >> 8];
	}
	else     // linear (uncorrected) color
	{
		r = (r * v1) >> 13; // 4-bit results
		g = (g * v1) >> 13;
		b = (b * v1) >> 13;
	}
   if (r>7) r=7;
   ...
   return Color444(r,g,b);
	return ((r & 0x7) << 12) | ((r & 0x8) << 8) | // 4/4/4 -> 5/6/5
	       ((g & 0x7) <<  7) | ((g & 0xC) << 3) |
	       ((b & 0x7) <<  1) | ( b        >> 3);
}

void RGBmatrixPanel4::drawPixel(int16_t x, int16_t y, uint16_t c)
{
	...
	if(y < (nRows * nMultiplexRows))
	{
		// Data for the upper half of the display is stored in the lower
		// bits of each byte.
		
		// Each base address has it's planes stored in 3 bytes, 32*nMultiplexRows*nPanels bytes apart (1 byte for 2 pixels: top/bottom). For 1/4 scan with 1 panel at relative addresses 0,64 and 128
		// Plane 0 is a tricky case -- its data is spread about, stored in least two bits not used by the other planes.
		ptr = ptr + (64 * nMultiplexRows * nPanels);		
		*ptr &= ~B00000011;            // Plane 0 R,G mask out in one op
		...
		ptr = ptr - (32 * nMultiplexRows * nPanels);
		...
		for(; bit < limit; bit <<= 1)
		{
			*ptr &= ~B00011100;             // Mask out R,G,B in one op
			...
			ptr  += 32 * nMultiplexRows * nPanels;                  // Advance to next bit plane
		}
		// planeCounter = 0;
		// Serial.println();
	...
	else
	{
		...
		*ptr &= ~B00000011;               // Plane 0 G,B mask out in one op
		if(r & 1)  ptr[32 * nMultiplexRows  * nPanels] |=  B00000010; // Plane 0 R: 32 bytes ahead, bit 1
		else       ptr[32 * nMultiplexRows  * nPanels] &= ~B00000010; // Plane 0 R unset; mask out
...
void RGBmatrixPanel4::fillScreen(uint16_t c)
{
	if((c == 0x0000) || (c == 0xffff))
	{
		...
		memset(matrixbuff[backindex], c, 32 * nMultiplexRows * nRows * 3 * nPanels);
	}
	else
	{
...
uint8_t *RGBmatrixPanel4::backBuffer()
{
...
void RGBmatrixPanel4::swapBuffers(boolean copy)
{
	if(matrixbuff[0] != matrixbuff[1])
	{
		...
		if(copy == true)
			memcpy(matrixbuff[backindex], matrixbuff[1 - backindex], 32 * nRows * nMultiplexRows * 3 * nPanels);
	}
}
...
void RGBmatrixPanel4::dumpMatrix(void) {
...
// Estimated timing for 1/4 panels?
/*   FIXME - Check this...
#define CALLOVERHEAD 60   // Actual value measured = 56
#define LOOPTIME     350  // Double the time of the 1/8 panel? Twice the pixels per row
*/
#if defined(__AVR__)
  #define CALLOVERHEAD 60   // Actual value measured = 56
...
#endif
#if defined(ARDUINO_ARCH_ESP32)
  #define CALLOVERHEAD 60  // Actual = 25
  #define LOOPTIME     800 // Actual = 1563 / 4
#endif
...
#if defined(ARDUINO_ARCH_ESP32)
IRAM_ATTR void RGBmatrixPanel4::updateDisplay(void) {
#else
void RGBmatrixPanel4::updateDisplay(void) {
#endif
	...
	*latport |= latpin; // Latch data loaded during *prior* interrupt
	...
	// of this method.
	t = (nRows > 8) ? LOOPTIME : (LOOPTIME * 2);		// Multiplier of 2 for 32x32pixel panel? Could also apply for 1/4 since double the row length
	...
	else if(plane == 1)
	{
		...
		if(nRows > 4) 	// Can be skipped for 1/4 scan panels, only 4 rows, C not used
      {
    ...
    #define pew                    \
      ...
      *outsetreg = sclkpin;
		...
		for (panelcount = 0; panelcount < nPanels; panelcount++)
		{
			...
			if (nMultiplexRows == 2) {
			// Another 32 bytes for the extra multiplexed rows
			pew pew pew pew pew pew pew pew
			pew pew pew pew pew pew pew pew
			pew pew pew pew pew pew pew pew
			pew pew pew pew pew pew pew pew
...
#if defined(ARDUINO_ARCH_SAMD) || defined(ARDUINO_ARCH_ESP32)
    *outclrreg = sclkpin; // Set clock low
#endif
//      buffptr = ptr; 
		buffptr += 32 * nMultiplexRows * nPanels;
/*	#else		// Code for none AVR (i.e. Duo and ARM based systems)
 
		for(i = 0; i < (32 * nMultiplexRows * nPanels ); i++)
		{
			DATAPORT = ptr[i];
			SCLKPORT = tick; // Clock lo
			SCLKPORT = tock; // Clock hi
		}
		buffptr += (32 * nMultiplexRows * nPanels);  // Move to next plane;
	#endif
*/
	...
	else     // 920 ticks from TCNT1=0 (above) to end of function
	{
		...
		for(i = 0; i < (32 * nMultiplexRows * nPanels); i++)
		{
			DATAPORT =
			    ( ptr[i]    << 6)                   |			// 	
			    ((ptr[i + (32 * nMultiplexRows * nPanels)] << 4) & 0x30) |
			    ((ptr[i + (64 * nMultiplexRows * nPanels)] << 2) & 0x0C);
			SCLKPORT = tick; // Clock lo
			SCLKPORT = tock; // Clock hi
...
// This commented out gets rid of shadowing on 1:4 displays.
/*    for (int i=0; i<WIDTH; i++) {
	...
	( ptr[i]         << 6)         |
      ...
      *outsetreg = sclkpin;    // Set clock high
    }
    *outclrreg = sclkpin;      // Set clock low
*/
#endif
